...
def get_public_property(code: str, db: Session = Depends(get_db)):
    ...
    if code.isdigit():
        prop = db.query(models.Property).filter(models.Property.id == int(code)).first()
        if prop: return prop
        
    prop = db.query(models.Property).filter(models.Property.code == code).first()
    if not prop:
...
def list_properties(
    ...
    query = db.query(models.Property).filter(models.Property.tenant_id == user.tenant_id)
    ...
    return query.order_by(models.Property.id.desc()).limit(limit).offset(offset).all()
...
def create_property(prop: schemas.PropertyCreate, db: Session = Depends(get_db), email: str = Depends(get_current_user_email)):
    user = db.query(models.User).filter(models.User.email == email).first()
...
def update_property(prop_id: int, data: schemas.PropertyCreate, db: Session = Depends(get_db), email: str = Depends(get_current_user_email)):
    user = db.query(models.User).filter(models.User.email == email).first()
    prop = db.query(models.Property).filter(models.Property.id == prop_id, models.Property.tenant_id == user.tenant_id).first()
    ...
    # Actualizar Pipeline IA
    sync_property_ai(db, prop)
...
def delete_property(prop_id: int, db: Session = Depends(get_db), email: str = Depends(get_current_user_email)):
    ...
    addr = prop.address
    db.delete(prop)
    db.add(models.ActivityLog(user_id=user.id, action="DELETE", entity_type="PROPERTY", entity_id=None, description=f"Eliminó propiedad: {addr}"))
    db.commit()
    return {"status": "ok"}

@router.patch("/{prop_id}")
def patch_property(prop_id: int, data: dict, db: Session = Depends(get_db), email: str = Depends(get_current_user_email)):
    user = db.query(models.User).filter(models.User.email == email).first()
    prop = db.query(models.Property).filter(models.Property.id == prop_id, models.Property.tenant_id == user.tenant_id).first()
    if not prop: raise HTTPException(404, "Propiedad no encontrada")
    ...
    # Trigger AI sync if description or title changed (though patch is mostly for images now)
